  ...
  var FormBuilder = function(options, element) {
    ...
    var defaults = {
      ...
      messages: {
        ...
        fieldRemoveWarning: 'Estas seguro que quieres remover este campo?',
    ...
    var advFields = function(values) {
      ...
      // Placeholder
      advFields.push(textAttribute('placeholder', { hidden: true }, values));
      // Class
    ...
    $sortableFields.on('click touchstart', '.delete-confirm', function(e) {
      ...
      let removeField = () => {
        $field.slideUp(250, function() {
          ...
          if($field.data('only-once')){ $('li[data-server-id='+ $field.data('server-id') +']').show(); }
        });
      };
